...
Slack のチャンネル投稿を Socket Mode で受け取り、`Q.` から始まるメッセージを生成サーバの prompt として実行します。`P.` から始まるメッセージでは prompt をモデル token ごとに表示し、次 token の raw softmax 上位 5 件を返します。解釈したメッセージには保留中の印として `:heartbeat:` リアクションを付け、結果を同じチャンネルへ投稿した後に外します。
